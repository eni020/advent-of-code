...
fun main() {
  ...
  var row = 0
  var maxColumn = 0
  File("year2023/day11/input11.txt").forEachLine { line ->
    if (maxColumn == 0) {
  ...
  val columnsWithGalaxies = galaxies.map { it.second }.distinct()
  val emptyRows = (0 until row).filter { !rowsWithGalaxies.contains(it) }
  val emptyColumns = (0 until row).filter { !columnsWithGalaxies.contains(it) }
